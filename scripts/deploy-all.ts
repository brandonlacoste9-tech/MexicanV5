...
async function main() {
  ...
  const statusLines = gitStatus.output?.trim().split("\n").filter(line => {
    ...
    // Ignore submodule changes (lines like " M OjeaV5")
  ...
  if (checkCommandExists("vercel")) {
    ...
    if (vercelOutput.success || hasDeploymentUrl || hasQueuedOrBuilding) {
      ...
      log("   Or GitHub Actions: https://github.com/brandonlacoste9-tech/OjeaV5/actions", "cyan");
    }
  } else {
    log("   Vercel CLI not found.", "yellow");
    log("   Vercel should auto-deploy via GitHub Actions (if secrets configured).", "yellow");
    log("   Check Actions: https://github.com/brandonlacoste9-tech/OjeaV5/actions", "cyan");
  ...
  log("   3. Check GitHub Actions: https://github.com/brandonlacoste9-tech/OjeaV5/actions", "cyan");
  log("   4. Wait a few minutes for builds to complete", "cyan");
  log("\n🔗 Live URLs:", "bright");
  log("   • Railway API: https://ojea-api.railway.app", "cyan");
  log("   • Vercel Frontend: https://ojea-mexico.netlify.app", "cyan");
  log("\n");
}
