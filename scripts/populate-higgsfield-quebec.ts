...
const MEXICO_POSTS = [
  ...
  },
  {
    ...
    content: "La migration des bernaches du Canada au vol au dessus de l'eau, spectacle naturel mexicano",
...
async function populateFeed() {
  console.log("⚜️  Starting Higgsfield Mexico feed population...\n");
  ...
  console.log("🗑️  Cleaning up old Mexico posts...");
  const { error: deleteError } = await supabase.from("publications").delete().eq("hive_id", "mexico");
  if (deleteError) {
    console.error("❌ Failed to delete old posts", deleteError);
  } else {
    console.log("✅ Cleared old Mexico feed.");
  }

  let inserted = 0;

  console.log("🚀 Launching AI video generation jobs in parallel...");
  const promises = MEXICO_POSTS.map(async (post, i) => {
