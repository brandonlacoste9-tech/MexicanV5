...
async function syncToRailway() {
  console.log("🚂 Starting Ojea Secret Sync to Railway...\n");
  ...
  // Read Vertex Key
  const vertexKeyPath = path.resolve(process.cwd(), "ojea-vertex-key.json");
  ...
  const aiKeyPath = path.resolve(process.cwd(), "ojea-ai-key.json");
  ...
  for (const [key, value] of Object.entries(variables)) {
    ...
    try {
      ...
      const command = `npx @railway/cli variables set -s "OjeaV5" "${key}=${value.replace(/"/g, '\\"')}"`;
      execSync(command, {
        stdio: "inherit",
