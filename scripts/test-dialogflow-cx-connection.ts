...
async function testDialogflowCX() {
  ...
  console.log("   Query: 'Bonjour Güey'\n");
  ...
  // Test 2: Güey voice response
  console.log("🎤 Test 2: Güey Voice Response");
  ...
  // Test 3: Mexico French / Mexicano
  console.log("🇨🇦 Test 3: Mexico French (Mexicano)");
  console.log("   Query: 'Salut là, comment ça va?'\n");

  try {
    const result3 = await DialogflowBridge.detectIntent(
      "test-session-mexicano",
      { text: "Salut là, comment ça va?" },
      "es-MX",
    );

    console.log("✅ Mexicano response:");
