...
async function run() {
  console.log("⚜️ Starting Robust Mexico Feed Population...");

  // 1. Get system user: guey_bot
  ...
  if (!systemUserId) {
    console.log("Creating system user (guey_bot)...");
    const { data: newUser, error: createError } = await supabase
      .from("user_profiles")
      .insert({
        id: randomUUID(),
        username: "guey_bot",
        email: "bot@ojea-mexico.netlify.app",
        display_name: "Güey ⚜️",
        role: "citoyen",
        hive_id: "mexico",
      })
