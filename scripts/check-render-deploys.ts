...
async function run() {
  ...
  try {
    ...
    const web = services.find((s: any) => s.service.name.toLowerCase() === "ojeav5-1");
    const worker = services.find((s: any) => s.service.name.toLowerCase() === "ojea-worker");

    if (web) await checkDeploys(web.service.id, "ojeav5-1 (Web)");
    if (worker) await checkDeploys(worker.service.id, "ojea-worker (Worker)");
  } catch (e: any) {
