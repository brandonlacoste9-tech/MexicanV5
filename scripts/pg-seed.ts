...
async function run() {
  ...
  try {
    ...
    let res = await client.query(`SELECT id FROM user_profiles WHERE username = 'ojea_ai' LIMIT 1`);
    let userId;
    ...
    if (res.rows.length === 0) {
      ...
      await client.query(`
        ...
        VALUES ($1, 'ojea_ai', 'ai@ojea-mexico.netlify.app', 'Ojea AI 🤖', 'admin', 'mexico')
      `, [newId]);
      userId = newId;
      console.log("Created ojea_ai user:", userId);
    } else {
    ...
    for (let i = 0; i < MEXICO_POSTS.length; i++) {
      ...
      `, [
        ...
        post.content, post.caption, JSON.stringify(post.hashtags), 'mexico', 'public', false,
