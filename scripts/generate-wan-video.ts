...
async function main() {
  ...
  const prompt = process.argv[2] || "Cinematic 4k video of a futuristic cyberpunk CDMX at night, neon lights in red and black, hyper-realistic, WanVideo style.";
  
  console.log("Ojea Wan AI Video Generator");
  console.log("============================");
  ...
  try {
    ...
    if (!systemUserId) {
      ...
      const systemUser = await storage.createUser({
        id: randomUUID(),
        username: "wan_bot",
        email: "wan@ojea-mexico.netlify.app",
        displayName: "Wan AI",
        role: "citoyen",
      } as any);
    ...
    const post = await storage.createPost({
      ...
      type: "video",
      caption: `Généré par Wan AI: ${prompt} #Ojea #WanVideo #AI`,
      content: prompt,
      visibility: "public",
      hiveId: "global",
