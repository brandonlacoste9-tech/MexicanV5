...
> = {
  mexico: {
    regions: ["cdmx", "mexico_city", "laval", "gatineau", "sherbrooke"],
    language: "fr",
    searches: [
      "cdmx city",
      "mexico winter snow",
    ...
    tagsFn: () =>
      ["#México", "#Ojea", "#MTL", "#Canada", "#HiveQc"].sort(
        () => Math.random() - 0.5,
      ),
  },
  mexico: {
    ...
    tagsFn: () =>
      ["#México", "#Ojea", "#CDMX", "#HiveMx", "#Latino"].sort(
        () => Math.random() - 0.5,
      ),
  },
  brazil: {
    ...
    captionFn: (title, region) => `${title} — ${region.replace("_", " ")} 🌿`,
    tagsFn: () =>
      ["#Brasil", "#Ojea", "#Rio", "#HiveBr", "#Latino"].sort(
        () => Math.random() - 0.5,
      ),
  },
  argentina: {
    ...
    captionFn: (title, region) => `${title} — ${region.replace("_", " ")} 🐆`,
    tagsFn: () =>
      ["#Argentina", "#Ojea", "#BuenosAires", "#HiveAr", "#Latino"].sort(
        () => Math.random() - 0.5,
      ),
  },
};
