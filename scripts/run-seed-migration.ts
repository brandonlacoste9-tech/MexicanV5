 ...
 * Executes migrations/0012_seed_initial_data.sql to populate database with Mexico posts
 */
...
async function runSeedMigration() {
  ...
  try {
    ...
    if (userCount === 0) {
      ...
      console.log("   Please create a user account at https://ojea-mexico.netlify.app/signup first.\n");
      await client.end();
      process.exit(1);
    }
    ...
    console.log("   This will insert 15 Mexico-themed posts into the publications table\n");
    ...
    const postCheck = await client.query(
      "SELECT COUNT(*) as count FROM publications WHERE hive_id = 'mexico'"
    );
    const postCount = parseInt(postCheck.rows[0].count);

    console.log("\n✅ Migration completed successfully!");
    console.log(`📊 Found ${postCount} Mexico posts in publications table`);
    ...
    const samplePosts = await client.query(
      "SELECT id, content, created_at FROM publications WHERE hive_id = 'mexico' ORDER BY created_at DESC LIMIT 5"
    );
