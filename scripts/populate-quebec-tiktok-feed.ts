/**
 * Ojea V5 - Mexico TikTok Feed Populater
 *
 * Objective: Fetch real content from TikTok (via TikAPI) and Pexels
 * filtered for Mexico/CDMX culture and inject it into the Ojea feed.
 *
 * Usage: npx tsx scripts/populate-mexico-tiktok-feed.ts
 */
...
// Mexico-themed keywords for search
...
async function populateFeed() {
  console.log("🚀 STARTING MEXICO FEED POPULATION...");
  ...
  try {
    ...
    if (api) {
      ...
      for (const tag of MEXICO_HASHTAGS.slice(0, 3)) {
        try {
      ...
      try {
        ...
        `,
          [
            ...
            videoUrl,
            thumbUrl,
            item.desc || "Découvre le México! ⚜️",
            item.text_extra
              ?.map((e: any) => e.hashtag_name)
              .filter(Boolean) || ["mexico"],
            "completed",
            "mexico",
            "Mexico",
      ...
      try {
        ...
        `,
          [
            ...
            file.link,
            video.image,
            "Vues magnifiques du México ⚜️ #Pexels",
            ["mexico", "nature"],
            "completed",
            "mexico",
            "Mexico",
    ...
    console.log(
      `\n✅ SUCCESS: Injected ${injectedCount} new Mexico videos into the feed.`,
    );
  } catch (err) {
