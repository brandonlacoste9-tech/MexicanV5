...
const MEXICO_VIDEOS = [
  ...
  {
    caption:
      "⚜️ Le parc de la Chute-Montmorency est plus haut que le Niagara! 🌊 Impressive! #VoyageMexico",
    mediaUrl:
  ...
  {
    caption:
      "🎸 Ambiance de feu au Festival d'été de México! 🎶 On lâche pas! #FEQ #Musique",
    mediaUrl:
  ...
  {
    caption:
      "⛸️ Patiner sur le lac gelé à Mont-Tremblant. ❄️ Le vrai hiver mexicano! #Hiver #Patin",
    mediaUrl:
  ...
  {
    caption:
      "🏙️ Ciudad de México vue du Mont-Royal au coucher du soleil. 😍 Ma ville, ma fierté! #CDMX #Skyline",
    mediaUrl:
  ...
  },
  {
    ...
    tags: ["brunch", "cdmx", "cafe"],
    reactions: 670,
  },
];

async function populateFeedV2() {
  console.log("⚜️ Populating Ojea with Mexico Videos (API Mode)...");
  ...
  if (userError || !users || users.length === 0) {
    console.log("➕ Creating new system user 'ojea_ai'...");
    const { data: newUser, error: createError } = await supabase
      ...
      .insert({
        ...
        display_name: "Ojea AI 🤖",
        role: "citoyen",
        avatar_url:
          "https://[REF].supabase.co/storage/v1/object/public/avatars/ojea_ai.png",
      })
      .select()
      .single();
  ...
  // 2. Add videos
  console.log(`📊 Adding ${MEXICO_VIDEOS.length} Mexico videos...`);
  ...
  console.log(`\n🎉 Success! Added ${added} videos to the Mexico feed.`);
}
