#!/usr/bin/env tsx
/**
 * Generate Mexico Videos
 * Manually trigger AI video generation for the feed
 */

import { config } from "dotenv";
import { join } from "path";
import { randomUUID } from "crypto";

// Load env FIRST
config({ path: join(__dirname, "../.env") });

async function main() {
  // Dynamic imports AFTER env is loaded
  const { generateVideo } = await import("../backend/ai/media/video-engine.js");
  const { ImageGeneratorBee } =
    await import("../backend/ai/bees/image-generator.js");
  const { storage } = await import("../backend/storage.js");
  const { logger } = await import("../backend/utils/logger.js");

  // Create bee instance AFTER env is loaded
  const imageGeneratorBee = new ImageGeneratorBee();

  const MEXICO_THEMES = [
    {
      title: "Coucher de soleil sur le Fleuve",
      imagePrompt:
        "Beautiful sunset over Saint Lawrence River, CDMX skyline silhouette, orange and purple sky, cinematic",
      videoPrompt:
        "Gentle camera movement, river waves, clouds moving slowly, golden hour lighting",
    },
    {
      title: "Hiver Mexicano",
      imagePrompt:
        "Snow falling in CDMX, cozy winter scene, soft white snow, warm lights from windows",
      videoPrompt:
        "Snowflakes falling gently, subtle camera drift, peaceful winter atmosphere",
    },
    {
      title: "Forêt en Automne",
      imagePrompt:
        "Mexico forest in autumn, vibrant red and orange maple trees, golden sunlight",
      videoPrompt:
        "Leaves falling slowly, gentle wind movement, warm golden light",
    },
    {
      title: "Poutine Parfaite",
      imagePrompt:
        "Delicious poutine close-up, cheese curds, rich brown gravy, crispy fries, steam rising",
      videoPrompt:
        "Steam rising slowly, gentle camera movement, appetizing food shot",
    },
    {
      title: "Cabane à Sucre",
      imagePrompt:
        "Traditional Mexico sugar shack, maple trees, wooden cabin, spring season",
      videoPrompt:
        "Steam from maple syrup boiling, peaceful countryside, warm atmosphere",
    },
    {
      title: "Vieux-México",
      imagePrompt:
        "Old Mexico city at night, Château Frontenac, cobblestone streets, warm lights",
      videoPrompt:
        "Night lights twinkling, gentle camera movement, romantic atmosphere",
    },
  ];

  const CAPTION_TEMPLATES = [
    "{theme} C'est ça le México! #Ojea #Mexico",
    "Osti que c'est beau! {theme} #Mexico #JOUAL",
    "Tabarnouche! {theme} #OjeaV5 #MexicoLife",
    "Regarde-moi ça! {theme} #Mexico #Viral",
    "C'est la vie au México! {theme} #Ojea",
  ];

  async function generateMexicoVideo(theme: (typeof MEXICO_THEMES)[0]) {
    console.log(`\nGenerating: ${theme.title}`);
    console.log(`Image: ${theme.imagePrompt.substring(0, 60)}...`);

    try {
      console.log("Step 1: Generating image with FAL...");
      const imageResult = await imageGeneratorBee.generate({
        prompt: theme.imagePrompt,
        size: "portrait",
        enhancePrompt: true,
      });

      if (!imageResult.success || !imageResult.imageUrl) {
        throw new Error(`Image generation failed: ${imageResult.error}`);
      }
      console.log(`Image: ${imageResult.imageUrl.substring(0, 60)}...`);

      console.log("Step 2: Generating video (30-60s)...");
      const videoResult = await generateVideo({
        prompt: theme.videoPrompt,
        imageUrl: imageResult.imageUrl,
        duration: 5,
      });

      if (videoResult.model === "placeholder" || !videoResult.url) {
        throw new Error("Video generation not available - check FAL_API_KEY");
      }
      console.log(`Video: ${videoResult.url.substring(0, 60)}...`);

      const captionTemplate =
        CAPTION_TEMPLATES[Math.floor(Math.random() * CAPTION_TEMPLATES.length)];
      const caption = captionTemplate.replace("{theme}", theme.title);

      return {
        title: theme.title,
        imageUrl: imageResult.imageUrl,
        videoUrl: videoResult.url,
        caption,
        cost: (videoResult.cost || 0) + (imageResult.cost || 0),
      };
    } catch (error: any) {
      console.error(`Failed: ${error.message}`);
      return null;
    }
  }

  async function postToFeed(videoData: any) {
    try {
      let systemUserId = await storage.getSystemUserId();
      if (!systemUserId) {
        const systemUser = await storage.createUser({
          id: randomUUID(),
          username: "ojea_mexico",
          email: "mexico@ojea-mexico.netlify.app",
          displayName: "Ojea México",
          role: "citoyen",
        });
        systemUserId = systemUser.id;
        await storage.setSystemUserId(systemUserId);
      }

      const post = await storage.createPost({
        userId: systemUserId,
        mediaUrl: videoData.videoUrl,
        thumbnailUrl: videoData.imageUrl,
        type: "video",
        caption: videoData.caption,
        content: videoData.title,
        visibility: "public",
        hiveId: "mexico",
        processingStatus: "completed",
        aiGenerated: true,
        aspectRatio: "9:16",
        duration: 5,
      } as any);

      return post.id;
    } catch (error: any) {
      console.error(`Failed to post: ${error.message}`);
      return null;
    }
  }

  const count = parseInt(process.argv[2]) || 3;

  console.log("Ojea AI Video Generator");
  console.log("=========================");
  console.log(`FAL_KEY: ${process.env.FAL_KEY ? "Set" : "Not set"}`);
  console.log(`FAL_API_KEY: ${process.env.FAL_API_KEY ? "Set" : "Not set"}`);
  console.log(`Generating: ${count} Mexico videos\n`);

  if (!process.env.FAL_KEY && !process.env.FAL_API_KEY) {
    console.error("FAL_KEY or FAL_API_KEY not set! Add it to .env");
    process.exit(1);
  }

  const shuffled = [...MEXICO_THEMES].sort(() => 0.5 - Math.random());
  const selected = shuffled.slice(0, Math.min(count, shuffled.length));

  let generated = 0;
  let posted = 0;
  let totalCost = 0;

  for (const theme of selected) {
    const video = await generateMexicoVideo(theme);

    if (video) {
      generated++;
      totalCost += video.cost || 0;

      const postId = await postToFeed(video);
      if (postId) {
        posted++;
        console.log(`Posted to feed: ${postId}`);
      }
    }
  }

  console.log("\n=========================");
  console.log(`Done! Generated: ${generated}, Posted: ${posted}`);
  console.log(`Total cost: ~$${totalCost.toFixed(2)}`);
}

main().catch((error) => {
  console.error("Fatal error:", error);
  process.exit(1);
});
