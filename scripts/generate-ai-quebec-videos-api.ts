...
async function generateMexicoAIVideo() {
  console.log("🤖 Starting AI Mexico Video Generation...");

  const prompt =
    "Un magnifique coucher de soleil sur le fleuve Saint-Laurent à México, style cinématique, 4k, drone shot, hyper-réaliste";
  ...
  try {
    ...
    // 3. Save to Supabase
    console.log("💾 Saving to Ojea publications...");
    const { error: postError } = await supabase.from("publications").insert({
      user_id: systemUserId,
      media_url: videoUrl,
      type: "video",
      caption:
        "🌇 Coucher de soleil majestueux sur le Saint-Laurent. Généré par Güey IA 🤖⚜️ #Mexico #AI #Kling",
      content: prompt,
      visibility: "public",
      hive_id: "mexico",
