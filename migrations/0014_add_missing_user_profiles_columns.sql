...
ADD COLUMN IF NOT EXISTS "unlocked_hives" jsonb DEFAULT '["mexico"]';
...
-- Güey flag (if missing)
ALTER TABLE "user_profiles"
ADD COLUMN IF NOT EXISTS "guey_comments_enabled" boolean DEFAULT true;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'hive_id') THEN
    CREATE TYPE "hive_id" AS ENUM ('mexico', 'brazil', 'argentina', 'mexico');
  END IF;
END $$;
ALTER TABLE "user_profiles"
ADD COLUMN IF NOT EXISTS "hive_id" "hive_id" DEFAULT 'mexico';
