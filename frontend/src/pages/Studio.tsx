...
import { generateCaptions, smartTrim } from "../services/videoService";
...
const STAGE_INFO: Record<
  UploadStage,
  { emoji: string; label: string; progress: number }
> = {
  idle: { emoji: "📤", label: "Prêt", progress: 0 },
  preview: { emoji: "👀", label: "Aperçu", progress: 0 },
  uploading: {
    emoji: "☁️",
    label: "Téléversement vers le nuage...",
    progress: 33,
  },
...
export default function Studio() {
  ...
  const isPublishing =
    stage === "uploading" || stage === "optimizing" || stage === "publishing";
        ...
        {isPublishing && (
          <div className="text-center py-16 animate-in fade-in duration-300">
            ...
            <div className="flex justify-center gap-2 mt-6">
              {(["uploading", "optimizing", "publishing"] as UploadStage[]).map(
                (s, i) => (
                  <div
                    key={s}
        ...
        {stage === "error" && (
          <div className="text-center py-16 animate-in fade-in duration-300">
            ...
            <p className="text-white/50 text-sm mb-6 max-w-xs mx-auto">
              {errorMessage ||
                "Quelque chose a mal tourné. Ta vidéo est encore là."}
            </p>
