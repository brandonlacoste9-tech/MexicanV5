    ...
    async function loadLeaderboard() {
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_APP_URL || ""}/api/leaderboard/weekly-tippers`,
          {
            headers,
          },
        );
    ...
    <div className="min-h-screen bg-black leather-overlay pb-20">
      ...
      <div className="max-w-md mx-auto p-4">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-black text-gold-400 mb-1">
            Top Tippers de la Semaine
          </h1>
          <p className="text-leather-300 text-sm">
            Le classement se réinitialise dans:{" "}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-gold-500/50 mt-4 text-sm font-semibold uppercase tracking-widest">
              Calcul des scores...
            </p>
          </div>
        ) : users.length === 0 ? (
          <div className="bg-leather-900 border border-white/10 rounded-2xl p-8 text-center mt-10">
            ...
            <h3 className="text-lg font-bold text-white mb-2">
              Aucun généreux cette semaine!
            </h3>
            <p className="text-leather-300 text-sm">
              Sois le premier à envoyer des Cennes pour prendre la couronne 👑
            </p>
          </div>
        ) : (
          <>
                ...
                >
                  <div className="absolute -top-14 w-16 h-16 rounded-full border-2 border-slate-300/80 p-0.5 overflow-hidden transition-transform group-hover:scale-110 shadow-[0_0_15px_rgba(203,213,225,0.4)]">
                    <Image
                      src={
                        top3[1].avatarUrl ||
                        `https://ui-avatars.com/api/?name=${top3[1].username}&background=random`
                      }
                      alt={top3[1].username}
                      objectFit="cover"
                    />
                  ...
                  <div className="bg-gradient-to-t from-slate-900 to-slate-800 border-t border-slate-400/50 w-full h-24 rounded-t-lg flex flex-col items-center justify-end pb-3">
                    <span className="text-slate-300 font-bold text-sm truncate w-full text-center px-1">
                      @{top3[1].username}
                    </span>
                    <span className="text-green-400 font-black text-sm">
                      {top3[1].totalCennes}¢
                    </span>
                    <div className="absolute -bottom-3 w-6 h-6 rounded-full bg-slate-800 border border-slate-400 flex items-center justify-center text-slate-300 font-bold text-xs shadow-lg z-10">
                      2
                    </div>
                  </div>
                </div>
              ...
              {top3[0] && (
                ...
                >
                  <div className="absolute -top-10 w-14 h-14 animate-bounce z-20">
                    <img
                      src="/assets/emojis/icon-crown.png"
                      alt="Crown"
                      className="w-full h-full object-contain filter drop-shadow-[0_0_8px_rgba(255,215,0,0.8)]"
                    />
                  </div>
                  <div className="absolute -top-20 w-20 h-20 rounded-full border-2 border-gold-400 p-0.5 overflow-hidden transition-transform group-hover:scale-110 shadow-[0_0_25px_rgba(212,175,55,0.6)] z-10">
                    <Image
                      src={
                        top3[0].avatarUrl ||
                        `https://ui-avatars.com/api/?name=${top3[0].username}&background=random`
                      }
                      alt={top3[0].username}
                      objectFit="cover"
                    />
                  ...
                  <div className="bg-gradient-to-t from-yellow-900/80 to-gold-600/40 border-t-2 border-gold-400 w-full h-32 rounded-t-lg flex flex-col items-center justify-end pb-4 relative overflow-hidden">
                    ...
                    <span className="text-gold-300 font-bold text-sm truncate w-full text-center px-1 z-10">
                      @{top3[0].username}
                    </span>
                    <span className="text-green-400 font-black text-base z-10">
                      {top3[0].totalCennes}¢
                    </span>
                    <div className="absolute -bottom-3 w-8 h-8 rounded-full bg-yellow-900 border-2 border-gold-400 flex items-center justify-center text-gold-400 font-black text-sm shadow-lg z-20">
                      1
                    </div>
                  </div>
                </div>
                ...
                >
                  <div className="absolute -top-12 w-14 h-14 rounded-full border-2 border-orange-700/80 p-0.5 overflow-hidden transition-transform group-hover:scale-110 shadow-[0_0_15px_rgba(194,113,33,0.4)]">
                    <Image
                      src={
                        top3[2].avatarUrl ||
                        `https://ui-avatars.com/api/?name=${top3[2].username}&background=random`
                      }
                      alt={top3[2].username}
                      objectFit="cover"
                    />
                  ...
                  <div className="bg-gradient-to-t from-orange-950 to-orange-900/80 border-t border-orange-700/50 w-full h-20 rounded-t-lg flex flex-col items-center justify-end pb-2">
                    <span className="text-orange-300/80 font-bold text-xs truncate w-full text-center px-1">
                      @{top3[2].username}
                    </span>
                    <span className="text-green-400 font-black text-xs">
                      {top3[2].totalCennes}¢
                    </span>
                    <div className="absolute -bottom-3 w-6 h-6 rounded-full bg-orange-950 border border-orange-700 flex items-center justify-center text-orange-400 font-bold text-xs shadow-lg z-10">
                      3
                    </div>
                  </div>
                </div>
