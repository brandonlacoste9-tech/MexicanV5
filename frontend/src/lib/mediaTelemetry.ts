...
export const mediaTelemetry = {
  ...
  recordThumbnailTimeout(context?: string) {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.increment) {
      (window as any).__ojea_analytics.increment("video_thumbnail_timeout", { context });
    }
  },
  ...
  recordCacheHit(url: string) {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.increment) {
      (window as any).__ojea_analytics.increment("video_cache_hit");
    }
  },
  ...
  recordCacheMiss(url: string) {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.increment) {
      (window as any).__ojea_analytics.increment("video_cache_miss");
    }
  },
  ...
  recordBufferStarvation() {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.increment) {
      (window as any).__ojea_analytics.increment("video_buffer_starvation");
    }
  },
  ...
  recordTimeToFirstFrame(url: string, ms: number) {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.timing) {
      (window as any).__ojea_analytics.timing("video_ttff", ms, url?.slice(0, 60));
    }
  },
  ...
  recordBufferReady(url: string, ms: number) {
    ...
    if (typeof window !== "undefined" && (window as any).__ojea_analytics?.timing) {
      (window as any).__ojea_analytics.timing("video_buffer_ready", ms, url?.slice(0, 60));
    }
  },
...
/**
 * Analytics wiring: set window.__ojea_analytics with { increment(name, opts?), timing(name, ms, label?) }
