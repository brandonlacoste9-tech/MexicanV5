  ...
  const handleWordClick = async (word: string, e: React.MouseEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/dictionary/lookup/${encodeURIComponent(cleanWord)}`,
      );
              ...
              {entry && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div>
                    ...
                    <p className="text-sm leading-relaxed text-stone-200 mt-1">
                      {entry.definition}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-4">
                    <div>
                      ...
                      <p className="text-xs text-gold-400 font-mono mt-1">
                        {entry.partOfSpeech}
                      </p>
                    </div>
                    {entry.safetyFlag !== "safe" && (
                      <div>
                        ...
                        <p className="text-xs text-red-400 font-mono mt-1 uppercase tracking-tighter">
                          {entry.safetyFlag} 🚨
                        </p>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {entry.synonyms.map((s, j) => (
                          <span
                            key={j}
                            className="text-[10px] bg-white/5 px-2 py-0.5 rounded-full text-stone-400 border border-white/5"
                          >
                            {s}
                          </span>
                        ))}
