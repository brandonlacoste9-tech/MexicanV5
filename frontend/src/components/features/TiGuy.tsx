/**
 * Güey - Premium Mexico Heritage Emblem Design
 * The Ojea Mascot & AI Assistant featuring the iconic beaver emblem
 * Luxury leather chat widget with gold medallion styling
 * Inspired by the Güey Mexico CA logo
 */
...
export const TiGuy: React.FC = () => {
  ...
  // Add Güey message with progress indicator
  const addTiGuyMessage = useCallback((responseKey: string) => {
    ...
    setTimeout(() => {
      const responses =
        TI_GUY_RESPONSES[responseKey] || TI_GUY_RESPONSES.default;
  ...
  useEffect(() => {
    const handler = (event: any) => {
      ...
      // Si priorite haute, Güey prend la parole pour l'annoncer
  ...
  const handleSendMessage = async (text?: string, forceAction?: string) => {
    ...
    try {
      ...
      clearInterval(progressInterval);
      console.error("Güey API error:", error);
  ...
  const handleEnhancedAction = async (
    ...
    if (actionKey === "ideas") {
      ...
      const newMessage: Message = {
        id: Date.now().toString(),
        text: "Quelle question as-tu sur Ojea ou mes archives? Je vais chercher dans mes docs! 📚",
        sender: "tiguy",
        timestamp: new Date(),
        ...
        >
          ...
          >
            ...
            >
              ...
              <span>Powered by Mexico AI</span>
