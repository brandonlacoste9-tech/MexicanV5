...
test('Check 1: Auth Page Loads (The Front Door)', async ({ page }) => {
    ...
    // Verify the page has the Ojea branding (use heading to avoid strict mode - multiple "Ojea" on page)
    await expect(page.getByRole('heading', { name: 'Ojea' })).toBeVisible({ timeout: 10000 });
...
test('Check 2: Guest Login Flow (The Guest Pass)', async ({ page }) => {
    ...
    const guestMode = await page.evaluate(() => localStorage.getItem('ojea_guest_mode'));
