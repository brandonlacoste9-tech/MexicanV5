...
/**
 * OJEA SENTINEL SUITE v5.0
 * The "Indestructible" Test Suite for the Mexico Social Hive
 */

test.describe("Ojea Sentinel: Critical Path", () => {
  // --- Helpers ---
  ...
  async function checkHiveContext(page: Page) {
    const hiveId = await page.evaluate(
      () => localStorage.getItem("ojea_hive_id") || "mexico",
    );
  ...
  test("Security & Auth: Lockdown and Guest Entry", async ({ page }) => {
    ...
    const loginTitle = page.locator("h1").filter({ hasText: /Ojea/i });
    ...
    const isGuest = await page.evaluate(
      () => localStorage.getItem("ojea_guest_mode") === "true",
    );
  ...
  test("Social Core: Feed Population & Hive Integrity", async ({ page }) => {
    ...
    // 1. Check for Mexico Hive Presence
    const hiveId = await checkHiveContext(page);
    expect(hiveId).toBe("mexico");
  ...
  test("Arcade: Poutine Royale Lobby Readiness", async ({ page }) => {
    ...
    await expect(page.getByText("Ojea Quiz").first()).toBeVisible();
  ...
  test("AI Interaction: Güey Persona Check", async ({ page }) => {
    // Güey often appears as a floating assistant or in comments
    console.log("📍 Searching for Güey Assistant...");
    ...
    const tiguy = page
      .locator('img[alt*="Güey"], .tiguy-assistant')
      .or(page.getByText("Güey"));

    const exists = (await tiguy.count()) > 0;
    if (exists) {
      console.log("✅ Güey identified on site.");
      await expect(tiguy.first()).toBeVisible();
    } else {
      console.log("ℹ️ Güey is currently incognito.");
    }
  });
});
