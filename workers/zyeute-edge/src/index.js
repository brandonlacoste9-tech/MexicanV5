/**
 * ojea-edge — Cloudflare Worker API edge cache + keep-warm in front of Render.
 *
...
const ORIGIN = "https://ojeav5-bg8t.onrender.com";
...
function cachedResponse(entry, edgeStatus, soft) {
  ...
  headers.set("X-Ojea-Edge", edgeStatus);
  headers.set(
    "Cache-Control",
...
async function handleFetch(request, env, ctx) {
  ...
  try {
    ...
    return Response.json(
      ...
      { status: 502, headers: { "X-Ojea-Edge": "ORIGIN_ERROR" } },
    );
  }

  const headers = applyCors(new Headers(upstream.headers));
  headers.set("X-Ojea-Edge", soft > 0 ? "MISS" : "BYPASS");
...
async function handleScheduled(env, ctx) {
  ...
  console.log("ojea-edge warm", JSON.stringify(results));
  return results;
}
