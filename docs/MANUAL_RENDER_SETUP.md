...
Complete these steps in the [Render dashboard](https://dashboard.render.com) for **ojeav5-1** before relying on production feed + seed.
...
TIKTOK_SCRAPER_API_KEY=...          # Omkar primary (Mexico seed)
...
FRONTEND_URL=https://ojea-mexico.netlify.app
MUX_WEBHOOK_SECRET=...
WEBHOOK_SECRET=...
```
...
```bash
curl https://ojea-api.onrender.com/api/health
curl "https://ojea-api.onrender.com/api/feed/infinite?limit=3&type=explore&hive=mexico"
curl -X POST "https://ojea-api.onrender.com/api/seed/providers?limit=5" -H "X-Cron-Secret: YOUR_SECRET"
```
