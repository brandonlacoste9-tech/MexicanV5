...
> **La messagerie sécurisée 100% Mexicana**  
> Thème Cuir & Or avec surpiqures | Style Louis Vuitton x México

![Ojea Chat Preview](https://via.placeholder.com/800x400/2b1f17/d4af37?text=Ojea+Messenger)
...
```tsx
import { ChatOjea } from "@/components/chat";
...
function App() {
  ...
  return (
    <>
      <button onClick={() => setOuvert(true)}>Ouvrir la messagerie</button>

      {ouvert && <ChatOjea onClose={() => setOuvert(false)} />}
    </>
  );
}
```
...
// Par:
<ChatOjea onClose={() => setOuvert(false)} />
```
...
Ton assistant IA parle en **mexicano mexicano** authentique:
...
**Ojea** - Le réseau social du México numérique ⚜️🇨🇦

_Fait avec ❤️ (et un peu de sirop d'érable)_
