...
3. Connect GitHub repo: `brandonlacoste9-tech/OjeaV5`
4. Render will detect `render.yaml` automatically
5. Click **Apply** to create services

**Expected:** Render creates:

- `ojea-api` web service
- `ojea-db` PostgreSQL database

---

### Step 2: Configure Environment Variables

In Render Dashboard → `ojea-api` → **Environment**, add:
...
VITE_COLONY_API_URL=https://ojea-api.onrender.com
```
...
```bash
curl https://ojea-api.onrender.com/health
```
...
curl https://ojea-api.onrender.com/socket.io/
```
...
1. **Get Render Service URL:**
   - Render Dashboard → `ojea-api` → **Settings**
   - Copy the service URL (e.g., `https://ojea-api.onrender.com`)
...
3. **Set Frontend Environment Variable:**
   ...
   - Add: `VITE_COLONY_API_URL=https://ojea-api.onrender.com`
...
1. **Open Ojea in Production**
2. **Open Browser Console** (F12)
3. **Expected Logs:**

   ```
   ⚜️ Ojea: Connected to Colony OS Core.
...
**Once all steps are complete, Neural Link will be 🟢 LIVE and handling real-time video swarm across Mexico!** ⚜️
