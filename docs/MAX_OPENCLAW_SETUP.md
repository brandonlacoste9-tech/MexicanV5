...
Max needs to authenticate with Ojea backend using the token we configured:
...
Max can execute these Ojea commands:
...
curl -H "Authorization: p8KXOOrrGHmOsJF5aKprjaytb8df156q" \
  https://ojeav5-production.up.railway.app/api/max/status
```
...
{
  "backend": {
    "url": "https://ojeav5-production.up.railway.app",
...
4. ⏳ Verify Max can execute Ojea commands
