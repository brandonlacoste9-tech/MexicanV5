...
- CLAUDE_CODE_MASTER_PLAN_MEXICO_TIKTOK.md - mission
- UI_COPY_MEXICO_FRENCH.md - language rules
...
# Mexico emblem
ls frontend/public/mexico-emblem.png  # Should exist, ~250KB
...
- Open app: https://ojeav5.vercel.app
- Go to Feed page
- Do videos play? If not, follow VIDEO_PLAYBACK_FIX_PLAN.md §3.5
- Are strings in Mexico French? If not, audit with UI_COPY_MEXICO_FRENCH.md §6
...
- **GitHub:** `brandonlacoste9-tech/OjeaV5`
- **URL:** https://github.com/brandonlacoste9-tech/OjeaV5
- **Branch:** `main`

**Starting point:**

- **Master plan (Mexico French + TikTok-style):** **`docs/CLAUDE_CODE_MASTER_PLAN_MEXICO_TIKTOK.md`** — single brief for language, UI copy, TikTok features, emblem, video, and implementation order.
...
**Language — Mexico French:**

- All user-facing text (UI labels, buttons, messages, placeholders, errors) must be in **México French** (français mexicano), not English or international French.
- **Copy reference:** Use **`docs/UI_COPY_MEXICO_FRENCH.md`** for ready-to-use strings (buttons, labels, toasts, errors), variables (`{username}`, `{count}`), and the mini style guide (tu, accents, no English, etc.). Match existing app tone (e.g. “Feux”, “Suivre”, “Vérifie ta connexion”, “Vidéo non disponible”).
...
- **.cursorrules** / **.cursor/rules/** – Cursor rules (e.g. Antigravity session, Ojea context).
...
- **Docker Hub CI:** Added `.github/workflows/docker-publish.yml` to build and push `brandontech/ojea-backend` and `brandontech/ojea-colony-worker` on push to `main` (when backend/Dockerfiles change) or manual run. Uses GitHub secrets `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`.
...
| Docker Build & Push workflow      | Done; uses secrets; pushes to `brandontech/ojea-*`.             |
...
- **Vercel:** `vercel.json` with API rewrite to `https://ojea-api.railway.app/api/$1` and CSP header including Mux domains.
- **Frontend API base URL:** In `frontend/src/services/api.ts`, `API_BASE_URL` is `https://ojeav5-production.up.railway.app` when not localhost. Note: different from vercel rewrite target `ojea-api.railway.app`; may need to align.
...
2. **Mux webhook**
   - Mux Dashboard: webhook URL points at the same backend the app uses (e.g. `ojeav5-production.up.railway.app` or `ojea-api.railway.app`).
...
**Language:** All new UI text (buttons, labels, tooltips, toasts) must be in **México French**. Use the strings and style guide in **`docs/UI_COPY_MEXICO_FRENCH.md`** (e.g. Suivre / Abonné·e(s), Ajouter / Enregistrer, Partager, Feux, Commentaires; toasts like “Vidéo enregistrée”, “Tu suis {username}”). Do not add English or international French strings.
...
| UI copy — México French (strings, toasts, style guide) | `docs/UI_COPY_MEXICO_FRENCH.md`                                                          |
| **Master plan — Mexico French TikTok-style takeover**  | **`docs/CLAUDE_CODE_MASTER_PLAN_MEXICO_TIKTOK.md`**                                      |
...
3. Document in `docs/UI_COPY_MEXICO_FRENCH.md`

### Scenario 3: Mexico Emblem Not Showing

**Symptoms**:

- Header or video cards don't show aguila emblem
- 404 error for `/mexico-emblem.png`

**Diagnosis**:

```bash
# Check if asset exists
ls frontend/public/mexico-emblem.png

# Check if asset is being copied to dist
ls frontend/dist/mexico-emblem.png  # After build
```

**Solutions**:

- **If file doesn't exist**: Asset was deleted
  - Restore from git: `git checkout HEAD -- frontend/public/mexico-emblem.png`
...
- **If file in dist but not loading**: Path issue in component
  - Update image src to `/mexico-emblem.png` (absolute path from public root)
