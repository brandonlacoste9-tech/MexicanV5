...
Keep the Ojea ecosystem agile while revenue is on-deck: local-first where possible, credits reserved for high-value production.

## Core principle

**Compute is currency.** If a task can be done with a regex or a local script, don't send it to the LLM. Save the "brains" for the Mexicano voice and the in-app experience.
...
- **GenAI App Builder (Google):** Use your **$1300 credit** only for high-value production:
  ...
  - Voice Güey layer (Mexicano, user-facing).
  - In-app Güey copilot (build Güey for free within this credit).
...
- **Agentic pruning:** Güey should only activate when explicitly triggered (e.g. FAB, "ask Güey", or `ojea-ui-ux` tag). No background polling or always-on context; prevents token drain.
...
- **Loi 25 Compliance-as-a-Service:** The Compliance Auditor (Mexico wording, design, Loi 25, a11y) can be licensed to other Mexico startups as a B2B product.
...
| Production voice  | GenAI App Builder (Mexicano)  | $1300 credit |
