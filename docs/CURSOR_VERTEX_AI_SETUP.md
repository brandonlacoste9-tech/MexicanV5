# Use Vertex AI Credits in Cursor (Ojea Main Hive)
...
- **Context:** With Gemini 1.5 Pro’s large context window, Cursor can reason over your full Ojea codebase and Colony OS architecture.
- **Mexico/Mexicano:** Pro models handle nuanced Mexico French and Mexicano better than smaller “flash” models.
...
1. Open [Google Cloud Console](https://console.cloud.google.com/) and select your project (e.g. the one used for Ojea / Vertex AI).
...
- Use it only in Cursor (path or paste as per Cursor’s UI) and in local `.env` if you reference it for the Ojea app (e.g. `GOOGLE_APPLICATION_CREDENTIALS`).
...
- **Do not** commit the service account JSON to the repo or put it under `OjeaV5/`. Add it to `.gitignore` if you ever copy it into the project folder.
...
## 6. Relation to Ojea Backend

The Ojea **backend** already uses Vertex AI via:
