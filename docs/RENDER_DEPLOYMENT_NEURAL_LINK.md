...
This guide walks you through deploying the Ojea backend to **Render** so that **Neural Link** (WebSocket/Socket.IO) can go from **🟡 READY** to **🟢 LIVE**.
...
| -------------------------- | ----------- | ------------------------------------------------------------- |
| **Backend (Railway)**      | 🟢 Live     | `https://ojeav5-production.up.railway.app`                  |
...
- **Cost-Effective:** Starter plan suitable for Ojea's current scale.
...
3. Connect your GitHub repo (`brandonlacoste9-tech/OjeaV5`)
...
DATABASE_URL=<auto-populated from ojea-db>
...
const COLONY_API_URL =
  ...
  (import.meta.env.PROD
    ? "https://ojea-api.onrender.com" // ✅ Change from Railway to Render
...
Open Ojea in production and check browser console:
...
2. **Test AI Requests:** Güey and MexicanoBee via `colonyLink.requestTiGuyResponse(...)`
