...
The current `App.tsx` is a minimal debug version (Login + MinimalFeed). The real feed (`LaOjea`) is not in the routes. Restore the full routing if you're testing the production flow.
