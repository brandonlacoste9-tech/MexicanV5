...
- **Backend URL mismatch**  
  Frontend `api.ts` uses `API_BASE_URL`: on non-localhost it’s hardcoded to `https://ojeav5-production.up.railway.app`. If your backend URL changed (e.g. new Railway service), update that constant or switch to `VITE_API_URL` and set it in Vercel/Railway.
...
- **Mux webhook URL**  
  In Mux Dashboard, webhook URL must be your public backend base + `/api/webhooks/mux` (e.g. `https://ojeav5-production.up.railway.app/api/webhooks/mux`). Use HTTPS.
