...
### 1. Start Muted (LaOjea.tsx, line ~330)
...
### 2. Tap-to-Unmute Banner (LaOjea.tsx, line ~1283)
...
### 3. Single Tap = Unmute → Play/Pause (LaOjea.tsx, handleVideoTap)
...
### 5. TikTok-Style Progress Bar (LaOjea.tsx, line ~885)
...
### 6. 2-Video DOM Limit (LaOjea.tsx, line ~782)
...
5. **Progress bar missing?**
   → Check that `onTimeUpdate` callback is wired from MuxVideoPlayer to LaOjea
   → The bar only shows on `index === currentIndex` and `post.type === "video"`

## Key Files

- `frontend/src/pages/LaOjea.tsx` — Main feed page with scroll, tap, and state logic
...
ojea-mexico.netlify.app/feed had zero interactions, so autoplay was blocked.

## AG splash stuck on "Initialisation Sécurisée AG"

If the feed never loads and you only see the gold AG spinner:

1. Wait 4 seconds — the app auto-bypasses the splash.
2. Tap **Accès Manuel Direct** or open `/feed?force=1`.
3. Check Render is awake: `curl https://ojea-api.onrender.com/api/health`
4. Add `?debug=1` and check console for `[FeedDiagnostic]`.

## Mux fallback to native player

If Mux fails for a post, La Ojea falls back to `VideoPlayer` with `hls_url` / `media_url`.
See `LaOjea.tsx` — `muxFallbackIds` state and `onError` on `MuxVideoPlayer`.
