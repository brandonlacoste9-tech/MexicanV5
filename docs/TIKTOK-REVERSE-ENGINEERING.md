# TikTok Reverse Engineering & Ojea Comparison — June 4, 2026

To ensure Ojea feels familiar and performant to users migrating from TikTok, we analyzed TikTok's web application across its core features. This document outlines TikTok's design patterns and compares them against Ojea's current implementation to identify areas for improvement.
...
### Ojea Gaps / Opportunities
- **Grid density:** Ojea should ensure its desktop grid dynamically scales (up to 6 columns) to match TikTok's scanning efficiency.
- **Stats styling:** Ensure Ojea uses the exact `Bold Number + Regular Text` format for quick readability.
...
### Ojea Gaps / Opportunities
- **Category Chips:** Ojea could implement a sticky horizontal scrolling list of categories at the top of the Discover/Search page.
...
### Ojea Gaps / Opportunities
- **Desktop Layout:** Ojea should ensure comments on desktop use a side-panel rather than a full-screen modal or center popup, keeping the video visible and playing.
...
- **Internal vs External:** Separate internal sharing (to Ojea connections) from external sharing into two distinct rows.
