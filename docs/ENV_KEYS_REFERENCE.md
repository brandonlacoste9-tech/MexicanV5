...
| **DEEPSEEK_API_KEY**    | DeepSeek chat (Güey / swarm)         | [DeepSeek](https://platform.deepseek.com)                    |
| **FAL_KEY**             | FAL.ai – Güey image/video generation | [FAL](https://fal.ai)                                        |
| **OPENWEATHER_API_KEY** | Weather bee (Güey)                   | [OpenWeather](https://openweathermap.org/api)                |
| **ELEVENLABS_API_KEY**  | Voice bee (Güey)                     | [ElevenLabs](https://elevenlabs.io)                          |
...
| **FROM_EMAIL**                  | Sender for emails                | e.g. `Ojea <noreply@yourdomain.com>`                   |
...
## México Or symbol (branding)

The header shows the México Or symbol (Águila with lion, MÉXICO OR, maple leaf) when this file exists:

**`frontend/public/mexico-or-symbol.png`**

Copy your symbol image there; it appears next to the Ojea logo on larger screens. If the file is missing, the symbol is hidden.
...
- Frontend calls `https://ojeav5-production.up.railway.app` when not on localhost; CORS must allow your Vercel domain if you use a custom domain.
