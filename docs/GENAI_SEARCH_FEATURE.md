...
- **Mexico-aware**: Understands Mexicano, locations, culture
- **Auto-expands queries**: "poutine" → poutine, food, Mexico cuisine, frites

**Example:**

```bash
curl -X POST /api/genai/search \
  -d '{"query": "chill spots in CDMX", "limit": 10}'
```
...
const results = await fetch("/api/genai/search", {
  method: "POST",
  body: JSON.stringify({
    query: "meilleure poutine à Ciudad de México",
    filters: { location: "cdmx", vibe: "food" },
  }),
});

// Returns: poutine reviews, food tours, restaurant recs
// All ranked by relevance to Mexico food culture
```
