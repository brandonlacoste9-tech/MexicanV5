# Claude Code Master Plan — French Mexico TikTok-Style Takeover

**One brief to rule them all.** Use this as the single source of truth for turning Ojea into a **México French, TikTok-style** social video app.

**Repo:** `brandonlacoste9-tech/OjeaV5`  
...
- **Language:** 100% user-facing text in **français mexicano** (casual, « tu », no English, no international French).
- **Product:** TikTok-style vertical video feed: follow, fire (like), comment, share, save; clear emojis; right-side or bottom action strip; México Or branding on video screens.
...
| Rule                   | Meaning                                                                                                                                                                                                         |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Mexico French only** | Every new label, button, toast, error, placeholder is in México French. Use **`docs/UI_COPY_MEXICO_FRENCH.md`** for strings and the mini style guide.                                                           |
...
| **Emblem**             | México Or (aguila + lion + « MÉXICO OR ») is the visual signature: header top-right + top-right of **video screen** (feed card + single-video view). Asset: `/mexico-emblem.png` (in `frontend/public/`). |
...
- **i18n**: Custom translation system with 4 locales (es-MX, pt-BR, es-AR, es-MX)
...
ojea-blue: #003399; /* Mexico Blue */
ojea-snow: #f8f9fa; /* Snow White */
ojea-alert: #dc3545; /* Alert Red */
ojea-hydro: #ffcc00; /* Hydro Yellow */
```
...
**File:** **`docs/UI_COPY_MEXICO_FRENCH.md`**
...
Implement or refine in this order. Every new UI string must come from or be added to **`docs/UI_COPY_MEXICO_FRENCH.md`**.
...
- [ ] **Swipe left** from the feed opens a **full-screen Agent view** (recommended: new route `/agent` or `/guey`). See **`docs/SWIPE_LEFT_AI_AGENT_DESIGN.md`** for full design and **Agent Screen IA** (§7–9).
...
- [ ] Copy: **Agent** / **Güey**, **Retour**, **Écris un message…**; all México French. Return: swipe right or **Retour** → feed.
...
### Pattern 3: Mexico Emblem Placement

**Location**: `VideoCard.tsx` (lines 218-228)

```tsx
{
  /* México Or emblem — top right of video screen, small */
}
{
  post.type === "video" && (
    <img
...
- Asset location: `frontend/public/mexico-emblem.png`
...
## 5. Branding — México Or emblem

- **Asset:** `/mexico-emblem.png` (gold aguila, lion, « MÉXICO OR »). Lives in `frontend/public/mexico-emblem.png`.
- **Placement (done):**
...
2. **Copy pass** — Replace any remaining English or international French with strings from **`docs/UI_COPY_MEXICO_FRENCH.md`**; add missing keys to that doc.
...
| Purpose                                     | File(s)                                                                           |
| ------------------------------------------- | --------------------------------------------------------------------------------- |
| UI copy (strings + style)                   | `docs/UI_COPY_MEXICO_FRENCH.md`                                                   |
...
| Güey / chat                               | `frontend/src/components/ChatButton.tsx`, `ChatModal.tsx`; `TiGuySwarmAdapter.ts` |

---

## 9. Success criteria

- [ ] All user-facing text in the app is México French (no English; no international French).
- [ ] Every new or touched string exists in or is added to **`docs/UI_COPY_MEXICO_FRENCH.md`** and follows the style guide.
...
- [ ] México Or emblem appears on header and on video screens (feed + full-screen), small and top-right.
