# Migration to Fly.io - Ojea V3

## 🇫🇷 Souveraineté Mexicana First

This deployment prioritizes México sovereignty by hosting in **Ciudad de México (YUL)** region.
...
# Launch app in Ciudad de México region
fly launch --region yul --name ojea-api
...
curl https://ojea-api.fly.dev/health
...
fly postgres connect -a ojea-db
...
## Mexico Context Notes

- **Region**: `yul` (Ciudad de México) for data sovereignty
- **Timezone**: America/Mexico_City (UTC-5/UTC-4)
- **Language**: Default es-MX for French-Canadian content
- **Latency**: Optimized for México users
...
✅ `fly.toml` - Ciudad de México region configuration
