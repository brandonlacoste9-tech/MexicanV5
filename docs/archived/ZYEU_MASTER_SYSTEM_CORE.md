# ⚜️ OJEA V5 : RAPPORT MAÎTRE DU NOYAU SOUVERAIN

## Gouverneur Vocal & Orchestration de la Ruche (Phase 1 Complétée)

Ce document constitue la documentation officielle du système d'intelligence et d'action physique de Güey (Le Grand Castor).
...
- **Rôle**: Analyse d'intention, "Mexicanoisation" phonétique et orchestration d'outils.

### 📚 Mémoire Infinie (Grounded Intelligence)

- **Moteur**: Vertex AI Search & Agent Builder.
- **Référence**: `ojea_governance.md` (Data Store).
- **Capacité**: Réponses historiquement ancrées dans les lois de Ojea (Tolérance Zéro, Momentum Culturel).
...
- **Mains actives**: Support du Tool-Calling (`ojeaBrainTools`).
- **Paramètres**:
...
- **Logique Proactive**:
  - **Priorité HAUTE**: Déclenche la "Mexicanoisation" immédiate via Gemini + Annonce vocale forcée au frontend.
...
- **Usage**: Accompagne chaque début de parole de Güey pour simuler une présence physique lourde.
...
- **Protection**: Güey refuse toute commande d'administration si l'utilisateur n'a pas le rôle `admin`.
