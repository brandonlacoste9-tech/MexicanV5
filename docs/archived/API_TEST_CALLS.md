# 🧪 API Test Calls for Ojea

## Production URLs

**Vercel Frontend:** https://ojea.vercel.app
**Railway Backend:** https://ojeav5-production.up.railway.app
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/health"
```
...
curl -X GET "https://ojeav5-production.up.railway.app/api/media-proxy?url=https%3A%2F%2Fvideos.pexels.com%2Fvideo-files%2F856973%2F856973-hd_1920_1080_30fps.mp4" \
  -H "Accept: video/mp4" \
  -I
```
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/posts?limit=5" \
...
```bash
curl -X POST "https://ojeav5-production.up.railway.app/api/tiguy/chat" \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/posts?type=video&limit=3" \
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/media-proxy?url=https%3A%2F%2Fassets.mixkit.co%2Fvideos%2Fpreview%2Fmixkit-ink-swirling-in-water-191-large.mp4" \
  -I
```
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/media-proxy?url=https%3A%2F%2Fstream.mux.com%2Ftest.m3u8" \
  -I
```
...
```bash
curl -X GET "https://ojeav5-production.up.railway.app/api/media-proxy?url=YOUR_HLS_URL_HERE" \
...
curl -X GET "https://videos.pexels.com/video-files/856973/856973-hd_1920_1080_30fps.mp4" \
  -I \
  -H "Origin: https://ojea.vercel.app"
```
...
1. **Check feed:** https://ojeav5-production.up.railway.app/api/posts?limit=5
2. **Check health:** https://ojeav5-production.up.railway.app/api/health
3. **Check TI-GUY:** https://ojeav5-production.up.railway.app/api/tiguy/status
