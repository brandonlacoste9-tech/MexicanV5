# Media Handling Playbook - Ojea v3
...
This playbook provides comprehensive guidance for handling media (images, videos, audio) in Ojea v3, including upload, storage, optimization, delivery, and edge cases.
...
**User Story:** As a user, I want to post a video of a Mexico festival so others can see the culture.
...
async function uploadVideoPost(
  ...
  return {
    videoUrl: supabase.storage.from("videos").getPublicUrl(videoData.path).data
      .publicUrl,
  ...
  while (uploadedBytes < file.size) {
    ...
    try {
      await supabase.storage
        .from("videos")
        .uploadChunk(path, chunk, uploadedBytes);
...
async function detectVideoCodec(file: File): Promise<string> {
  ...
  return codec === "probably" || codec === "maybe"
    ? "supported"
    : "unsupported";
}
```
