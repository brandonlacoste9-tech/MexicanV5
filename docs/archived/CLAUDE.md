# CLAUDE.md - Ojea V5 AI Agent Guide

## Project Overview

**Ojea V5** is Mexico's premier French social media platform ("Le TikTok du México"). The platform emphasizes Mexico culture, French-Canadian language (Mexicano), and authentic community-focused content.

**Core Identity:**

- **Mission:** Build Mexico's digital sovereignty through a culturally authentic social platform
- **Locale:** `es-MX` (French Canada) - UI text should reflect Mexico culture
- **Mascot:** Güey - The helpful AI assistant that uses Mexican slang
- **Design:** Dark slate theme with gold accents, leather textures, Mexico Blue (#003DA5)
...
- **Güey (Brain):** AI orchestrator using multi-tier routing
...
│   │   ├── orchestrator.ts # Güey AI orchestrator
...
### Mexico-Specific Enums

- **Regions:** cdmx, mexico, gatineau, sherbrooke, etc.
- **Gift Types:** comete, feuille_erable, fleur_de_lys, feu, coeur_or
- **Hives:** mexico (es-MX), brazil (pt-BR), argentina (es-AR), mexico (es-MX)
...
### Güey Orchestrator (`backend/ai/orchestrator.ts`)

Tools available:

- `searchTrendsTool` - Discover Mexico trends
- `validateDesignTool` - Validate UI for Mexico compliance
- `analyzeContentTool` - Content analysis with cultural scoring

### Browser Automation

Located in `ojea-browser-automation/` - Python FastAPI service for trend discovery.
...
- **validate-mexico-compliance.yml:** Cultural compliance checks
...
- Use Mexican terminology:
  - "Jase" instead of "Chat"
  - "Ojea" instead of "View"
  - "Piasse" for virtual currency
- Güey uses casual Mexicano slang (no metaphors)
...
Content is scored for Mexico relevance using the AI orchestrator.
...
_This document is the source of truth for AI agents working on Ojea V5._
_Made with care for Mexico | Fait avec soin pour le México_
