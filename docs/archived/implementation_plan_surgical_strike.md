...
This plan focuses on bypassing the industrial complexity (Mux/BullMQ) to get Ojea live immediately with a functional video feed and simple user uploads.
...
- [ ] **Task T4**: Implementation: Receive file -> Upload to Supabase `ojea-videos` bucket -> Create DB record with raw URL -> Return success.
