# 🚀 Ojea V5 - Google Cloud Integration (Free Credits Strategy)
...
gcloud iam service-accounts create ojea-ai \
 --description='Ojea AI Service Account' \
 --display-name='Ojea AI'

# Grant Vertex AI permissions

gcloud projects add-iam-policy-binding unique-spirit-482300-s4 \
 --member='serviceAccount:ojea-ai@unique-spirit-482300-s4.iam.gserviceaccount.com' \
 --role='roles/aiplatform.user'

# Download key

gcloud iam service-accounts keys create ojea-ai-key.json \
 --iam-account=ojea-ai@unique-spirit-482300-s4.iam.gserviceaccount.com
`
...
GOOGLE_APPLICATION_CREDENTIALS=./ojea-ai-key.json
...
// Mexico-specific prompt enhancement
export const generateMexicoContent = async (
topic: string,
tone: 'formal' | 'casual' | 'humorous' = 'casual'
): Promise<string> => {
const mexicoPrompt =
Écris en français mexicano authentique sur:
Ton:
Utilise des expressions mexicanas comme 'c'est ben correct', 'tabarnak', etc.
Sois culturellement approprié pour le México.
;

return await generateText(mexicoPrompt);
};
`
...
const imagePrompt =
Génère une image pour le contenu suivant, en style mexicano/culturel:

Style: Photographie réaliste, couleurs vives, atmosphère mexicana
Format:
;
...
### **Translation Service (Mexico French Optimization)**
...
export const translateToMexicoFrench = async (text: string): Promise<string> => {
const [response] = await translationClient.translateText({
parent: projects//locations/global,
contents: [text],
mimeType: 'text/plain',
sourceLanguageCode: 'en', // or auto-detect
targetLanguageCode: 'es-MX', // Mexico French
});
...
` ypescript
// Advanced Mexico content generation
export const generateMexicoSocialPost = async (
topic: string,
...
const prompt =
Crée un post en français mexicano authentique sur:

Inclue:

- Caption engageante avec emojis
- 5-10 hashtags mexicano
...
- **Mexico-optimized content** generation
...
6. **Scale Mexico content** generation

**Your $300+ free credits can supercharge Ojea's AI capabilities!** 🚀
