# UI Copy : México French (pour Claude)

**Ready-to-copy** strings for the app. Everything is in **français mexicano**, matches existing app wording, and uses the agreed tone (casual, concise, « tu »).
...
| Clé                     | México French (déjà en place) | Commentaire                                       |
| ----------------------- | ----------------------------- | ------------------------------------------------- |
...
| Situation                     | Message México French                                | Variables    |
| ----------------------------- | ---------------------------------------------------- | ------------ |
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| **Toujours utiliser le « tu »**          | « Tu suis », « Vérifie ta connexion »                                                                                          | Ton familier, typique du México.                             |
...
| **Utiliser les accents**                 | « Échec », « Vérifie », « À propos »                                                                                           | Respect de l’orthographe mexicana.                         |
| **Pluralisation**                        | Libellé fixe reste **Feux** ; si besoin de « 1 feu » / « 2 feux », utiliser une clé séparée (`like_singular` / `like_plural`). | Le libellé affiché reste _Feux_ ; comptes dynamiques à part. |
...
**À retenir :** Chaque nouveau libellé doit passer par ce fichier (ou le même jeu de clés) pour garder toute la UI **uniforme en français mexicano**.
...
The app uses a **custom translation hook**, NOT react-i18next. This is simpler for the Mexico-focused approach.
...
export function useTranslation() {
  ...
  const t = (key: string) => {
    return TRANSLATIONS[locale]?.[key] || TRANSLATIONS["es-MX"][key] || key;
  };
  return { t, locale };
}
```
...
**Step 2**: Add to this doc (`UI_COPY_MEXICO_FRENCH.md`)
...
**Référence :** `docs/HANDOVER_VIDEO_AND_DEPLOY.md` §0 (Language — Mexico French) et §5 (TikTok-style social features).  
