...
```bash
# Database
DATABASE_URL=postgresql://user:pass@localhost:5432/ojea
...
ALERT_EMAILS=admin@ojea-mexico.netlify.app
...
# Run migration
Shell(cd OjeaV5 && psql $DATABASE_URL -f backend/config/messaging-schema.sql)

# Install deps
Shell(cd OjeaV5 && npm install)

# Start dev server
Shell(cd OjeaV5 && npm run dev)

# Check logs
Shell(cd OjeaV5 && tail -f logs/app.log)
```
