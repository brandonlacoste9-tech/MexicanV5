...
This guide will help you set up **Google Cloud Memorystore for Redis** for your Ojea application. Memorystore is Google's fully managed Redis service that provides:
...
2. **Create Instance**
   ...
   ```
   Instance ID:        ojea-redis
   Display name:       Ojea Redis Cache
...
gcloud redis instances create ojea-redis \
  --size=1 \
  --region=us-central1 \
...
gcloud redis instances create ojea-redis-prod \
  --size=5 \
  --region=us-central1 \
  --redis-version=redis_7_0 \
  --tier=standard \
...
2. Click on your instance (`ojea-redis`)
...
Connection Name:     projects/YOUR_PROJECT/locations/us-central1/instances/ojea-redis
```
...
gcloud redis instances describe ojea-redis --region=us-central1

# Get just the host IP
gcloud redis instances describe ojea-redis \
  --region=us-central1 \
  --format='get(host)'

# Get the auth string (password)
gcloud redis instances get-auth-string ojea-redis \
  --region=us-central1
```
...
gcloud run deploy ojea-backend \
  --source=./backend \
  --region=us-central1 \
...
```bash
gcloud compute networks vpc-access connectors create ojea-connector \
  --network=default \
  --region=us-central1 \
  --range=10.8.0.0/28
```
...
```bash
gcloud run services update ojea-backend \
  --vpc-connector=ojea-connector \
  --region=us-central1
```
...
```bash
# Get the auth string
gcloud redis instances get-auth-string ojea-redis --region=us-central1
...
# Check backend logs
gcloud run logs read ojea-backend --region=us-central1 --limit=50
```
...
gcloud redis instances update ojea-redis \
  --region=us-central1 \
  --update-auth
```
...
gcloud redis instances create ojea-redis-ha \
  --size=5 \
  --region=us-central1 \
  --redis-version=redis_7_0 \
  --tier=standard \
