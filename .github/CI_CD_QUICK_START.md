...
docker build -f backend/Dockerfile -t brandontech/ojea-backend:local .
docker build -f infrastructure/colony/Dockerfile -t brandontech/ojea-colony-worker:local infrastructure/colony
```
...
1. **Backend Image**
   ...
   - Tagged as `brandontech/ojea-backend:latest`
   - Also tagged with commit SHA: `brandontech/ojea-backend:main-<sha>`

2. **Colony Worker Image**
   - Built from `infrastructure/colony/Dockerfile`
   - Tagged as `brandontech/ojea-colony-worker:latest`
   - Also tagged with commit SHA: `brandontech/ojea-colony-worker:main-<sha>`
...
Add these in: [GitHub Settings → Secrets](https://github.com/brandonlacoste9-tech/ojea-v3/settings/secrets/actions)
...
- `brandontech/ojea-backend:latest` (and tagged with SHA)
- `brandontech/ojea-colony-worker:latest` (and tagged with SHA)
...
Images: `brandontech/ojea-backend`, `brandontech/ojea-colony-worker`. Triggered on push to `main` when backend/ or Dockerfiles change, or via **Actions → Docker Build & Push → Run workflow**.
>>>>>>> main
...
**Team**: Ojea Development  
**Status**: ✅ Production Ready

🎭⚜️ **Ojea - L'app sociale du México**
